...
import { adaptPlanesPagoData } from "../../adapters/planes/PlanesPagoAdapter";
...
export const HomePageContent = () => {
    ...
    useEffect(() => {
        setSelectedCustomers(category ? selectedCustomers.filter(customer => customer.category.toLowerCase() === category.toLowerCase()) : selectedCustomers);
    }, [category])

    if (reading) {
        return (
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <CircularProgress sx={{ mt: 5, color: primary.main }} />
            </Box>
        );
    }
    ...
    const handleSheetsData = (sheetsData: SheetData[]) => {
        ...
        const planesPago = adaptPlanesPagoData(sheetsData.find((sheet) => sheet.sheetName === "Planes de Pago")?.data as any[], [...responsablesInscriptos, ...monotributistas]);

        setCustomers(adaptHonorariosData(sheetsData.find((sheet) => sheet.sheetName === "Honorarios")?.data as any[], planesPago))
    }
    ...
    const handleSendWhatsApp = () => {
        console.log("Enviar WhatsApp", selectedCustomers);
    }

    const handleSelectedCustomers = (customers: Customer[]) => {
            ...
            <Grid2 container spacing={2} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Grid2 size={{ xs: 12 }}>
                    <Stack spacing={2} direction={'row'}>
                        ...
                        {
                            sheetsData.length > 0 && selectedCustomers.length > 0 &&
                            <Button variant="text"
                                color="success"
                                startIcon={<SvgIcon component={WhatsApp} color="success" />}
                        ...
                        <Stack direction="row"
                            ...
                            <OutlinedInput
                                ...
                                startAdornment={
                                    <InputAdornment position="start">
                                        <SvgIcon component={SearchIcon} color="primary" />
                                    </InputAdornment>
                                }
                            ...
                            <Tooltip title="Filtrar clientes">
                                <IconButton onClick={(event) => handleClick(event)}>
                                    <SvgIcon component={FilterAltIcon} color="primary" />
                                </IconButton>
                            </Tooltip>
                        </Stack>
                        <CustomersTableContainer customers={category ? customers.filter(customer => customer.category.toLowerCase() === category.toLowerCase()) : customers}
                            search={search}
                            selectedCustomers={selectedCustomers}
                            setSelectedCustomers={handleSelectedCustomers}
                        />
                    </Grid2>
                }
